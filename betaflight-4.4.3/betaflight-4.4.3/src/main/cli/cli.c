...
static void cliMotorMix(const char *cmdName, char *cmdline)
{
    ...
    if (isEmpty(cmdline)) {
        ...
        ptr = cmdline;
        uint32_t j = atoi(ptr); // get mixer number
        if (j > 2) {
            cliShowArgumentRangeError(cmdName, "MIXER INDEX", 0, 2);
            return;
        }
        ptr = nextArg(ptr);
        ...
        if (i < MAX_SUPPORTED_MOTORS) {
            ptr = nextArg(ptr);
            if (ptr) {
                getCustomMotorMixerMutableByIndices(j,i)->throttle = fastA2F(ptr);
                check++;
            }
            ptr = nextArg(ptr);
            if (ptr) {
                getCustomMotorMixerMutableByIndices(j,i)->roll = fastA2F(ptr);
                check++;
            }
            ptr = nextArg(ptr);
            if (ptr) {
                getCustomMotorMixerMutableByIndices(j,i)->pitch = fastA2F(ptr);
                check++;
            }
            ptr = nextArg(ptr);
            if (ptr) {
                getCustomMotorMixerMutableByIndices(j,i)->yaw = fastA2F(ptr);
                check++;
            }
            if (check != 4) {
                cliShowInvalidArgumentCountError(cmdName);
            } else {
                printMotorMix(DUMP_MASTER, getCustomMotorMixerByIndices(j,0), NULL, NULL);
            }
        } else {
...
static void printConfig(const char *cmdName, char *cmdline, bool doDiff)
{
    ...
    if ((dumpMask & DUMP_MASTER) || (dumpMask & DUMP_ALL)) {
        ...
        if (!(dumpMask & HARDWARE_ONLY)) {
            ...
            printMotorMix(dumpMask, customMotorMixer_1_CopyArray, customMotorMixer_1(0), mixerHeadingStr);
            printMotorMix(dumpMask, customMotorMixer_2_CopyArray, customMotorMixer_2(0), mixerHeadingStr);
